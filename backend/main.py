...
# Route to get CBC reports for a specific patient
@app.get("/cbc-reports/patient/{patient_id}")
def get_user_cbc_reports(patient_id: int, db: Session = Depends(get_db)):
    reports = db.query(models.CBCReport).filter(models.CBCReport.patient_id == patient_id).all()
    if not reports:
        raise HTTPException(status_code=404, detail="CBC reports not found for this patient")
    return reports
